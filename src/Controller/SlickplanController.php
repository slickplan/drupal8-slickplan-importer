...
class SlickplanController
{
    ...
    public function addMedia($url, array $attrs = array())
    {
        ...
        try {
            if ($file) {
                $file = file_save_data($file, 'public://' . $attrs['file_name'], FILE_EXISTS_RENAME);
                if ($file and isset($file->uri)) {
    ...
    public function getSummaryRow(array $page)
    {
        ...
        if (isset($page['files']) and is_array($page['files']) and count($page['files'])) {
            ...
            $html .= '<div style="border-left: 5px solid rgba(0, 0, 0, 0.05); margin-left: 5px; ' . 'padding: 5px 0 5px 11px;">Files:<br />' . implode('<br />',
                    $files) . '</div>';
        }
        $html .= '<div>';
        return $html;
                ...
                switch ($type) {
                    ...
                    case 'image':
                        if (isset($element['content']['type'], $element['content']['url'])) {
                            $attrs = array(
                                'alt' => isset($element['content']['alt']) ? $element['content']['alt'] : '',
                                'title' => isset($element['content']['title']) ? $element['content']['title'] : '',
                                'file_name' => isset($element['content']['file_name']) ? $element['content']['file_name'] : ''
                            );
                    ...
                    case 'file':
                        if (isset($element['content']['type'], $element['content']['url'])) {
                            $attrs = array(
                                'description' => isset($element['content']['description']) ? $element['content']['description'] : '',
                                'file_name' => isset($element['content']['file_name']) ? $element['content']['file_name'] : ''
                            );
                            if ($element['content']['type'] === 'library') {
                                $src = $this->addMedia($element['content']['url'], $attrs);
    ...
    {
        $first_test = ($array and is_array($array) and isset($array['title'], $array['version'], $array['link']) and is_string($array['link']) and strstr($array['link'],
                'slickplan.'));
        if ($first_test) {
            if ($parsed) {
